...
document.getElementById('verify-challenge').addEventListener('click', function clicked (event) {
  ...
  if (selectDirBtn) {
    ...
    if (path === '') {
      ...
      // introduce a verify function that takes in two arguments
      // so we can handle the results asynchronously
      if (verifyChallenge.length === 2) {
        verifyChallenge(path, function (result) {
          // if the verify method returns a result, let's try and render it
          if (result) {
            for (var i = 0; i < result.length; i++) {
              var r = result[i]
              addToList(r.message, r.result)
            }

            if (result.some(r => r.result === false)) {
