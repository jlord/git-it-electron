...
function buildHeader (filename) {
  ...
  var source = fs.readFileSync(path.join(__dirname, '../partials/chal-header.html')).toString()
...
function buildFooter (file) {
  ...
  var data = getPrevious(num)
  data.lang = lang ? '-' + lang : ''
  var source = fs.readFileSync(path.join(__dirname, '../partials/chal-footer.html')).toString()
  var template = Handlebars.compile(source)
