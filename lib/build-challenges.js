...
var rawFiles = path.join(__dirname, (lang ? '../raw-content-' + lang + '/' : '../raw-content/'))
...
function makeShortname (filename) {
  // BEFORE guide/raw-content/10_merge_tada.html
  // AFTER  merge_tada
...
function buildFooter (file) {
  ...
  var data = getPrevious(num)
  data.lang = lang ? '-' + lang : ''
  var source = fs.readFileSync(path.join(__dirname, '../partials/chal-footer.html')).toString().trim()
  var template = Handlebars.compile(source)
...
function buildBody (file) {
  ...
  var content = {
    ...
    // challenge_completed: fs.readFileSync(path.join(__dirname, '../partials/verify-button.html')).toString()
  }
